    ...
    for idx, route in enumerate(route_list):
        if route[1] > 0:
            ...
            for connection in cave_list[route[0][-1]]:
                if (connection.islower() and connection not in route[0]) or not connection.islower():
                    new_route = route[0] + [connection]
                    if new_route not in [x[0] for x in route_list]:
                        route_list.append([new_route, 1])
                        s += 1
            if s == 0:
                route_list[idx][1] = 0
    count = sum([ x[1] for x in route_list])

filtered_list = [ route[0] for route in route_list if route[0][-1] == 'end']
print(filtered_list)
